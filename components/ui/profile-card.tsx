...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
	...
	const effectiveInnerGradient = customInnerGradient || innerGradient;
	const effectiveBehindGlowEnabled =
		showBehindGlow !== undefined ? showBehindGlow : behindGlowEnabled;
	const effectiveIconUrl =
		showIcon === true && !iconUrl
			? '/a4.svg'
			: showIcon === false
				? undefined
				: iconUrl;
	...
	const handlePointerEnter = useCallback(
		(event: PointerEvent): void => {
			...
			if (wrap) wrap.style.setProperty('--card-opacity', '1');
	...
	const handlePointerLeave = useCallback((): void => {
		...
		if (wrap) wrap.style.setProperty('--card-opacity', '0');
		tiltEngine.toCenter();
	...
	const cardStyle = useMemo(
		() => ({
			...
			'--inner-gradient':
				effectiveInnerGradient ?? DEFAULT_INNER_GRADIENT,
			'--behind-glow-color':
	...
	return (
		...
		>
			...
			>
				<section
					className="grid relative overflow-hidden h-[55svh] max-h-[400px] md:h-[80svh] md:max-h-[540px] aspect-[0.718]"
					style={{
						borderRadius: cardRadius,
						backgroundBlendMode:
